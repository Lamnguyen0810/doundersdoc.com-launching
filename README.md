...
Founders Doc's AI contract assistant. This repository contains the **demo** (`demo.py` — no database, runs anywhere)
and the scaffold of the **full product** (`app/`).

## Upload to GitHub and run on Render (no git, no terminal)

1. **GitHub** → https://github.com/new → name `fdai`, Private, tick *Add a README*, Create.
2. On the repo page **Add file → Upload files**. In File Explorer open this folder, press **Ctrl+A**, drag everything
   onto the page. Folders (`app`, `demo`, `tests`, `alembic`, `docs`, `.github`) keep their structure. Commit.
   Check the repo shows `demo.py`, `Dockerfile`, `app/` and `demo/` at the top level.
3. **Render** → https://render.com → sign in with GitHub → **New + → Blueprint** → choose `fdai` → it reads
   `render.yaml` → paste your `ANTHROPIC_API_KEY` when asked → **Apply**.
   (Or **New + → Web Service**, Docker, region Singapore, Free, and add `ANTHROPIC_API_KEY` yourself.)
4. Three to five minutes later the service is **Live**. Open the URL:

| URL | Page |
|---|---|
| `/` | Founders Doc site (Mockup E "Robin") — every **Launch FD AI** → `/fd-ai` |
| `/fd-ai` | FD AI landing page — dropzone reviews a contract; **Open FD AI** → `/chat` |
| `/chat` | FD AI, ChatGPT-style: draft contracts (download .docx), review or ask about an attached document |
| `/app` | Documents and reviews this session |

Updating later: **Add file → Upload files** again with the changed files; Render redeploys automatically.
Free instances sleep after 15 minutes idle — open the URL a minute before you present.
...
Architecture and rationale: [`docs/FD AI_Backend_Proposal.md`](docs/FD AI_Backend_Proposal.md).

## Demo today — no database, no accounts

```bash
pip install -r requirements.txt
export ANTHROPIC_API_KEY=sk-ant-...   # optional: without it you get a canned review
uvicorn demo:app --reload             # http://localhost:8000 = the FD AI landing page, dropzone wired to the backend
```
...
| URL | Page |
|---|---|
| `/` | Founders Doc site (Mockup E) — every **Launch FD AI** goes to `/fd-ai` |
| `/fd-ai` | FD AI landing page; dropzone posts to `/documents`; **Open FD AI** → `/chat` |
| `/chat` | FD AI chat — free-form: draft contracts (download as .docx), review or ask about an attached document, streaming replies |
| `/app` | Documents this session |

Focus for now is **Review** (upload → grade, issues, email) and **Draft** (chat → contract → .docx). Everything else is scaffolded in `app/` for later.

Everything is in memory. The plan for turning this into the product is in [`PLAN.md`](PLAN.md).

## What works in the full app (week-1 scaffold)
...
```bash
git init && git add -A && git commit -m "FD AI backend scaffold"
